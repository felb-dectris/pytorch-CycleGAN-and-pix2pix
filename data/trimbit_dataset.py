...
class TrimbitDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.image_paths = sorted(self.make_dataset(self.data_dir, opt.max_dataset_size,))
    ...
    def make_dataset(self,dir, max_dataset_size=None, energies=None):
            print(energies)
            if energies is None:
                energies=[8000]
            ...
            for root, _, fnames in sorted(os.walk(dir)):
                if any(is_image_file(fname) for fname in fnames):
                    vrfp = int(root.split('/')[-1])
                    if energies is not None and vrfp not in energies:
                        continue
                    images.append(root)
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(A_path, 'energies.json'),'r') as f:
            energies = json.load(f)
        images = {}
        for i, energy in enumerate(energies):
            fpath = os.path.join(A_path, f'{i}.npy')
            img = Image.fromarray(numpy.load(fpath).reshape(256,1024))
            images[i] = img
